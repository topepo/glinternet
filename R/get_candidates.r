get_candidates <- function(X, Z, res, n, pCat, pCont, numLevels, interactionPairs, categoricalCandidates, continuousCandidates, screenLimit = NULL, activeSet = NULL, norms = NULL, numCores = 1) {
  ...
  if (pCat > 0) {
    ...
    if (is.null(norms$cat)) {
      candidates$norms$cat <- compute_norms_cat(X, res, n, pCat, numLevels, numCores)
    } else {
      candidates$norms$cat <- norms$cat
    }
  }
  if (pCont > 0) {
    candidates$variables$cont <- matrix(1:pCont, ncol = 1)
    if (is.null(norms$cont)) {
      candidates$norms$cont <- compute_norms_cont(Z, res, n)
    } else {
      candidates$norms$cont <- norms$cont
    }
  }
